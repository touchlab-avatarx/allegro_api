...
{
  return (frame_id << 2) | (device_id & 0x03);
}

inline canid_t rtr_frame_id(canid_t frame_id, canid_t device_id)
{
  return ((frame_id) << 2) | (device_id & 0x03) | CAN_RTR_FLAG;
}
...
AllegroHand::Implementation::Implementation() : is_running_(false), socket_(0)
{
...
void AllegroHand::Implementation::init(const std::string& port, int id)
{
  port_ = port;
  device_id_ = id;
  init_can();
...
void AllegroHand::Implementation::set_torque(const std::vector<double>& torque)
{
...
void AllegroHand::Implementation::get_imu(std::vector<double>& quaternion)
{
  std::unique_lock<std::mutex> guard(get_mutex_);
  quaternion = imu_;
  guard.unlock();
...
void AllegroHand::Implementation::update()
{
  ...
  while (is_running_)
  {
    if (can_read(socket_, frame))
    {
      ...
      canid_t device_id = frame.can_id & 0x03;
      if (device_id != device_id_) continue;
      switch (can_id)
      {
...
void AllegroHand::Implementation::init_can()
{
...
void AllegroHand::Implementation::can_servo(bool enable)
{
  can_frame frame;
  frame.can_id = standard_frame_id(enable ? ID_CMD_SYSTEM_ON : ID_CMD_SYSTEM_OFF, device_id_);
  frame.can_dlc = 0;
  can_write(socket_, frame);
}

void AllegroHand::Implementation::can_set_torque(int index, const std::vector<int16_t>& torque)
{
  ...
  frame.can_id = standard_frame_id(ID_CMD_SET_TORQUE_1 + index, device_id_);
  frame.can_dlc = 8;
  can_write(socket_, frame);
}

void AllegroHand::Implementation::can_set_position(int index, const std::vector<int16_t>& position)
{
  ...
  frame.can_id = standard_frame_id(ID_CMD_SET_POSE_1 + index, device_id_);
  frame.can_dlc = 8;
  can_write(socket_, frame);
}

void AllegroHand::Implementation::can_set_period(const std::vector<int16_t>& period)
{
  if (period.size() != 3) THROW("Invalid period vector size");
  can_frame frame;
  frame.can_id = standard_frame_id(ID_CMD_SET_PERIOD, device_id_);
  frame.can_dlc = 6;
...
void AllegroHand::Implementation::can_set_device_id(uint8_t id)
{
  can_frame frame;
  frame.can_id = standard_frame_id(ID_CMD_CONFIG, device_id_);
  frame.can_dlc = 6;
...
void AllegroHand::Implementation::can_set_rs485_baudrate(uint32_t rate)
{
  can_frame frame;
  frame.can_id = standard_frame_id(ID_CMD_CONFIG, device_id_);
  frame.can_dlc = 6;
...
void AllegroHand::Implementation::can_request_information()
{
  can_frame frame;
  frame.can_id = rtr_frame_id(ID_RTR_HAND_INFO, device_id_);
  frame.can_dlc = 0;
  can_write(socket_, frame);
}

void AllegroHand::Implementation::can_request_serial()
{
  can_frame frame;
  frame.can_id = rtr_frame_id(ID_RTR_SERIAL, device_id_);
  frame.can_dlc = 0;
  can_write(socket_, frame);
}

void AllegroHand::Implementation::can_request_position(int index)
{
  if (index < 0 || index >= NUM_OF_FINGERS) THROW("Invalid finger index for requesting position");
  can_frame frame;
  frame.can_id = rtr_frame_id(ID_RTR_FINGER_POSE + index, device_id_);
  frame.can_dlc = 0;
  can_write(socket_, frame);
}

void AllegroHand::Implementation::can_request_imu()
{
  can_frame frame;
  frame.can_id = rtr_frame_id(ID_RTR_IMU_DATA, device_id_);
  frame.can_dlc = 0;
  can_write(socket_, frame);
}

void AllegroHand::Implementation::can_request_temperature(int index)
{
  if (index < 0 || index >= NUM_OF_FINGERS) THROW("Invalid finger index for requesting position");
  can_frame frame;
  frame.can_id = rtr_frame_id(ID_RTR_TEMPERATURE + index, device_id_);
  frame.can_dlc = 0;
  can_write(socket_, frame);
}
